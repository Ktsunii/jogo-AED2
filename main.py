...
class SobreviventeInsalubre:
    ...
    def _serializar_no_avl(self, no: NoAVL, dados: List):
        """Função auxiliar recursiva para serializar a AVL em uma lista."""
        if no:
            # 1. Serializa o nó da esquerda
            self._serializar_no_avl(no.esquerda, dados)
            
            # 2. Serializa o nó atual (os dados que queremos salvar)
            dados.append({
            ...
            # 3. Serializa o nó da direita
            self._serializar_no_avl(no.direita, dados)

    def salvar_ranking(self):
        """Salva a estrutura da AVL em um arquivo JSON."""
        dados_ranking = []
        self._serializar_no_avl(self.ranking, dados_ranking)
        
        # Cria o diretório 'data' se não existir
        os.makedirs('data', exist_ok=True)
        persistence_path = os.path.join('data', 'ranking.json')
        
        try:
            with open(persistence_path, 'w', encoding='utf-8') as f:
                json.dump(dados_ranking, f, indent=4)
            print(f"✅ Ranking da AVL salvo em: {persistence_path}")
        except IOError as e:
            print(f"❌ Erro ao salvar o ranking: {e}")

    def carregar_ranking(self):
        """Carrega os dados do ranking de um arquivo JSON e reconstrói a AVL."""
        persistence_path = os.path.join('data', 'ranking.json')
        ...
        if os.path.exists(persistence_path):
            try:
                ...
                # Reinsere os dados na AVL para reconstruir a estrutura balanceada
                for jogador_data in dados_carregados:
                    # Para simplificar, vamos usar uma função de reconstrução mais simples:
                    self.ranking = self.reconstruir_no_avl(self.ranking, jogador_data)
                    
                print(f"✅ Ranking da AVL carregado de: {persistence_path}")
            except (IOError, json.JSONDecodeError, KeyError) as e:
                print(f"❌ Erro ao carregar o ranking: {e}")
        else:
            print("ℹ️ Arquivo ranking.json não encontrado. Iniciando com ranking vazio.")

    def reconstruir_no_avl(self, no: NoAVL, data: dict) -> NoAVL:
        """Função auxiliar para reconstruir um nó da AVL (mais simples que inserir_avl)."""
        if not no:
            novo_no = NoAVL(data["nome_jogador"], data["pontuacao_recorde"])
            novo_no.historico_avls = data.get("historico_avls", [])
            novo_no.contador_mortes = data.get("contador_mortes", 0)
            novo_no.record_eventos = data.get("record_eventos", 0)
            novo_no.chefes_derrotados = data.get("chefes_derrotados", 0)
            novo_no.total_avls = data.get("total_avls", 0)
            return novo_no
        
        # Para fins de reconstrução, apenas inserimos na estrutura básica
        if data["nome_jogador"] < no.nome_jogador:
            no.esquerda = self.reconstruir_no_avl(no.esquerda, data)
        elif data["nome_jogador"] > no.nome_jogador:
            no.direita = self.reconstruir_no_avl(no.direita, data)
        else:
            # Caso de nomes repetidos (deveria ser evitado)
            if data["pontuacao_recorde"] > no.pontuacao_recorde:
                no.pontuacao_recorde = data["pontuacao_recorde"]
            no.historico_avls = data.get("historico_avls", [])
            no.contador_mortes = data.get("contador_mortes", 0)
            no.record_eventos = data.get("record_eventos", 0)
            no.chefes_derrotados = data.get("chefes_derrotados", 0)
            no.total_avls = data.get("total_avls", 0)
        
        # Nota: Idealmente, após a inserção (ou reconstrução), a AVL deveria ser rebalanceada.
        # Por simplicidade e foco na persistência, mantemos a inserção básica aqui.
